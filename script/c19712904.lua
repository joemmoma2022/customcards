...
function s.initial_effect(c)
	-- Special Summon from hand by tributing 2 WATER monsters if "Umi" or "Big Umi" is on the field
	...
	-- Once per turn: Target 1 opponent's monster; it loses 500 ATK per WATER in GY, then inflict 500 damage
...
function s.spcon(e,c)
	...
	return Duel.IsExistingMatchingCard(s.umifilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil)
		and Duel.CheckReleaseGroup(tp,Card.IsAttribute,2,nil,ATTRIBUTE_WATER)
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local count=Duel.GetMatchingGroupCount(Card.IsAttribute,tp,LOCATION_GRAVE,0,nil,ATTRIBUTE_WATER)
	if tc and tc:IsFaceup() and tc:IsRelateToEffect(e) and count>0 then
		local atk_down = count * 500
		-- Decrease ATK
